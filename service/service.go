...
func doServerStuff(conn net.Conn, num int) {
	...
	for {
		//读取客户端发来消息
		c, err := conn.Read(clientInfo)
		if err != nil {
			fmt.Println(" connection error: ", err)
			return
		}
		fmt.Println(clientInfo[:c]) //string(clientInfo))
		sInfo := "hello " + "this is " + strconv.Itoa(count) + "\n"
		clientInfo = []byte(sInfo) //向客户端发送消息
